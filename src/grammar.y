...
%}
 ...
 | direct_declarator '(' { ++indentLvl; isFunction = true ; typeLock = true ; declarationFunction = true; pushSymbolTable(symbolTable); } parameter_type_list ')' { free(saveFunctionName);saveFunctionName=copy($<s>1);typeLock = false;  atExitDeclaration($<s>1);}
| direct_declarator '(' { ++indentLvl; isFunction = true ;typeLock = true; declarationFunction = true; pushSymbolTable(symbolTable); } ')' { free(saveFunctionName);saveFunctionName=copy($<s>1);typeLock = false;  atExitDeclaration($<s>1);}
| direct_declarator '(' { ++indentLvl; isFunction = true ;typeLock = true; declarationFunction = true; pushSymbolTable(symbolTable); } identifier_list ')' { free(saveFunctionName);saveFunctionName=copy($<s>1); typeLock = false; atExitDeclaration($<s>1);}
 ;
