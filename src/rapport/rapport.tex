...
Ainsi nous avons une liste de types commun à tous nos fichiers. Cependant les types dénifit dans des librairies externes (exemple : \textit{bool}, \textit{size_t}, \textit{FILE}...)
...
Chaque type rencontré (ainsi que les '*') est sauvegardé dans une variable, il sera donné plus tard à \textit{functionParser}.
\\
...
\\
En considérant le point abordé précédemment, on peut dire qu'aucune déclaration ayant le même nom se trouve au même niveau d'indentation. Il suffit alors de stocker la classe d'un \textit{identifier} uniquement au niveau d'indentation (ou niveau de pile) 0.
\end{itemize}
